...
## Creare una pipeline in Progettazione e caricare i dati nell'area di disegno
...
1. In [studio di Azure Machine Learning, nel riquadro sinistro selezionare l'elemento **Progettazione** (in **Creazione**) e quindi selezionare **+** per creare una nuova pipeline.](https://ml.azure.com?azure-portal=true)
...
1. Trovare il **modulo Select Columns in Dataset (Seleziona colonne nel set di dati** ) e inserirlo nell'area di disegno sotto il **set di dati diabetes-data** . Connettere quindi l'output dalla parte inferiore del set di dati **diabetes-data** all'input nella parte superiore del modulo **Selezionare le colonne nel set di dati**.

1. Fare doppio clic sul modulo **Select Columns in Dataset** per accedere a un riquadro delle impostazioni a destra. Selezionare **Modifica colonna**. Quindi nella **finestra Seleziona colonne** selezionare **Per nome** e **Aggiungi tutte le** colonne. Quindi rimuovere **PatientID** e fare clic su **Salva**.

1. Trovare il **modulo Normalize Data (Normalizza dati** ) e posizionarlo nell'area di disegno sotto il **modulo Select Columns in Dataset (Seleziona colonne nel set di dati** ). Connettere quindi l'output dalla parte inferiore del modulo **Selezionare le colonne nel set di dati** all'input nella parte superiore del modulo **Normalizzazione dei dati**, come illustrato di seguito:
...
1. **Nella pagina Informazioni di base** selezionare **Crea nuovo** e impostare il nome dell'esperimento su **mslearn-diabetes-training** e quindi selezionare **Avanti**.
...
1. Attendere qualche minuto per il completamento dell'esecuzione. È possibile controllare lo stato del processo selezionando **Processi** in **Asset**. Da qui selezionare l'esperimento **mslearn-diabetes-training** e quindi il **processo Diabetes Training** .

## Visualizzare i dati trasformati

Al termine dell'esecuzione, il set di dati viene preparato per il training del modello.

1. Fare clic con il pulsante destro del mouse (CTRL+clic su un Mac) il **modulo Normalize Data (Normalizza dati** ) nell'area di disegno e selezionare **Anteprima dati**. Selezionare **Set di dati trasformato**.
...
1. Tornare alla **pagina Progettazione** e selezionare la **pipeline Diabetes Training** .
...
1. Selezionare **Configura e invia** ed eseguire la pipeline usando l'esperimento esistente denominato **mslearn-diabetes-training**.

1. Attendere il completamento del processo di esecuzione dell'esperimento. Il processo può richiedere 5 o più minuti.

1. Controllare lo stato del processo selezionando **Processi** in **Asset**. Da qui, selezionare l'esperimento **mslearn-diabetes-training** e quindi selezionare il processo di training** diabetes più recente**.

1. Nella nuova scheda fare clic con il pulsante destro del mouse (CTRL+clic su un Mac) il **modulo Score Model (Punteggio modello**) nell'area di disegno, selezionare **Anteprima dati e quindi selezionare Set di dati**** con punteggio per visualizzare **i risultati.

1. Scorrere verso destra e osservare che accanto alla colonna **Diabetic**, contenente i valori effettivi dell'etichetta, è presente una nuova colonna denominata **Scored Labels**, contenente i valori delle etichette stimate, e una colonna **Scored Probabilities**, contenente un valore di probabilità compreso tra 0 e 1. Ciò indica la probabilità di una previsione *positiva*, quindi le probabilità maggiori di 0,5 generano un'etichetta equivalente a ***1*** (diabetico), mentre quelle tra 0 e 0,5 generano un'etichetta equivalente a ***0*** (non diabetico).

1. Chiudere la **scheda Scored_dataset** .

Il modello stima i valori per l'etichetta **Diabetic**, ma quanto sono affidabili queste stime? Per capirlo, è necessario valutare il modello.
...
1. Tornare a **Progettazione** e aprire la **pipeline Diabetes Training** creata.
...
1. Connessione l'output del **** Assegnare un punteggio al *modulo Dataset1* (più a sinistra) di **Execute Python Script (Esegui script** Python) e connettere l'output *del set di dati* Risultato (a sinistra) del **modulo Execute Python Script (Esegui script** Python) all'output** del **servizio Web.
...
1. Tornare alla  **scheda Processi** . Selezionare quindi l'esperimento **mslearn-diabetes-inference** e quindi selezionare il **processo Predict Diabetes** .
...
1. Nella parte superiore della **finestra Predict Diabetes job (Stimare il diabete** ) selezionare **Deploy (Distribuisci**).
...
1. **In Configura endpoint in tempo reale selezionare **Deploy new real-time endpoint** (Distribuisci nuovo endpoint** in tempo reale) e usare le impostazioni seguenti:
    - **Nome**: predict-diabetes
    - **Descrizione**: Classify diabetes
    - **Tipo di** calcolo: Istanza di Azure Container

1. Selezionare **Distribuisci** e attendere la distribuzione del servizio Web. L'operazione può richiedere alcuni minuti.
