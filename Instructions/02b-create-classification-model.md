...
1. In **Impostazioni**, in **Dettagli bozza** modificare il nome della bozza (**Pipeline-Created-on-* date***) in **Diabetes Training**.
...
1. Fare clic su **Dati**. Cercare il set di dati **diabetes-data** e posizionarlo sul canvas.

1. Fare clic con il pulsante destro del mouse (CTRL+clic su un Mac) sul set di dati **diabetes-data** nel canvas e fare clic su **Anteprima dei dati**.
...
1. Chiudere la scheda di **visualizzazione dei risultati dei dati sul diabete** in modo che sia possibile visualizzare il set di dati nell'area di disegno come segue:
...
1. Selezionare **Invia** ed eseguire la pipeline come un nuovo esperimento denominato **mslearn-diabetes-training** nel cluster di elaborazione.

1. Attendere qualche minuto per il completamento dell'esecuzione.

    ![Screenshot della libreria della finestra di progettazione con il pulsante seguente del processo e dei dettagli del processo segnato come completato.](media/create-classification-model/completed-job.png)
...
1. Fare clic con il pulsante destro del mouse (CTRL+clic su un Mac) sul modulo **Normalize Data** nel canvas e fare clic su **Anteprima dei dati**. Selezionare **Set di dati trasformato**.
...
1. Aprire la pipeline **Diabetes Training** creata nell'unità precedente, se non è già aperta.
...
1. Selezionare **Invia** ed eseguire la pipeline usando l'esperimento esistente denominato **mslearn-diabetes-training**.

1. Attendere il completamento del processo di esecuzione dell'esperimento. Il processo può richiedere 5 o più minuti.

1. Al termine dell'esperimento, selezionare **Dettagli processo**. Si verrà reindirizzati a una nuova scheda.

1. Nella nuova scheda fare clic con il pulsante destro del mouse (CTRL+clic su un Mac) sul modulo **Score Model** sul canvas e scegliere **Anteprima dei dati**. Selezionare **Set di dati con punteggio** per visualizzare i risultati.
...
1. Nell'elenco a discesa **Create inference pipeline** fare clic su **Real-time inference pipeline**. Dopo alcuni secondi, verrà aperta una nuova pipeline denominata **Diabetes Training-real time inference**.

1. Passare a **Impostazioni** nel menu in alto a destra. In **Dettagli della bozza** assegnare alla nuova pipeline il nome **Predict Diabetes** e riesaminarla. Alcune trasformazioni e passaggi di training fanno parte di questa pipeline. Il modello sottoposto a training verrà usato per assegnare un punteggio ai nuovi dati. La pipeline contiene anche un output del servizio Web che restituisce i risultati. 
...
1. Selezionare **Dettagli processo** nel riquadro a sinistra. Verrà aperta un'altra finestra.

    ![Screenshot dei dettagli del processo vicino al processo completato. ](media/create-classification-model/completed-job-inference.png)

1. Nella nuova finestra selezionare **Distribuisci**.
...
1. In alto a destra selezionare **Distribuisci** e distribuire un **nuovo endpoint in tempo reale** usando le impostazioni seguenti: 
