...
Il servizio *Visione artificiale* usa modelli di Machine Learning con training preliminare per analizzare le immagini ed estrarre informazioni su di esse.
...
1. Non preoccuparti troppo del codice, l'aspetto importante è che necessita dell'URL dell'endpoint e di una delle chiavi per la risorsa dei servizi di intelligenza artificiale di Azure. Copiare questi valori dalla pagina **Chiavi ed endpoint** per la risorsa dal portale di Azure e incollarli nell'editor di codice, sostituendo rispettivamente i valori segnaposto **YOUR_KEY** e **YOUR_ENDPOINT**.
