...
1. Modificare il nome della bozza (**Pipeline-Created-on-date***) in **Train Penguin Clustering(Eseguire il training di Penguin Clustering**).
...
1. Tornare alla pipeline selezionando **Finestra di progettazione** nel menu a sinistra. Nella pagina Progettazione selezionare la bozza della pipeline Train Penguin Clustering.On the **Designer** page, select the **Train Penguin Clustering** pipeline draft.
...
1. Selezionare **Dati**, cercare e inserire il **set di dati penguin-data** nell'area di disegno.
...
1. Selezionare la *scheda Profilo* , notando che è possibile visualizzare le distribuzioni delle varie colonne come istogrammi.

1. Di seguito sono descritte le caratteristiche del set di dati.

    - Il set di dati include le colonne seguenti:
        - **CulmenLength**: lunghezza della fattura del pinguino in millimetri.
        - **CulmenDepth**: profondità della fattura del pinguino in millimetri.
        - **FlipperLength**: lunghezza della flipper del pinguino in millimetri.
...
1. Chiudere la **pagina DataOutput** per visualizzare il set di dati nell'area di disegno della pipeline.
...
1. Selezionare **Salva**, quindi chiudere il **menu Seleziona colonne nel set di dati** per tornare all'area di disegno della finestra di progettazione.

1. **Nella libreria** asset cercare un **modulo Clean Missing Data (Pulisci dati** mancanti) e inserirlo nell'area di disegno, sotto il **modulo Select columns in dataset (Seleziona colonne nel set di dati**) e connetterle come segue:
...
1. Fare doppio clic sul **modulo Pulisci dati** mancanti e nel riquadro delle impostazioni a destra selezionare **Modifica colonna**. Nella finestra **Colonne da pulire** selezionare **Con regole** e includere **Tutte le colonne**, come illustrato di seguito:
...
1. Selezionare **Salva** e quindi nel riquadro delle impostazioni impostare le impostazioni di configurazione seguenti:
...
1. Selezionare **Salva** e quindi chiudere le **impostazioni del modulo Normalize Data (Normalizza dati** ) per tornare all'area di disegno della finestra di progettazione.
...
1. Nella **pagina Informazioni di base** selezionare **Crea nuovo** e impostare il nome dell'esperimento su **mslearn-penguin-training** e quindi selezionare **Avanti** .
...
1. Attendere il completamento dell'esecuzione. Il processo può richiedere 5 o più minuti. È possibile controllare lo stato del processo selezionando **Processi** in **Asset**. Selezionare quindi il processo Train Penguin Clustering (Esegui training **penguin clustering** ).


## Visualizzare i dati trasformati

1. Al termine dell'esecuzione, i moduli avranno un aspetto simile al seguente:
...
1. Fare clic con il pulsante destro del **mouse sul modulo Normalize Data (Normalizza dati), selezionare **Preview data** (Anteprima dati**), quindi selezionare **Transformed dataset (Set di dati** trasformati) per visualizzare i risultati.
...
1. Tornare alla **pagina Progettazione** e aprire la bozza della **pipeline Train Penguin Clustering** .
...
1. Il modello di cui si esegue il training userà le caratteristiche per raggruppare i dati in cluster ed è quindi necessario eseguire il training del modello usando un algoritmo di *clustering*. **Nella libreria** asset cercare e inserire un **modulo K-Means Clustering nell'area di disegno**, a sinistra del **modulo Split data (Divisione dati**) e sopra il modulo Train Clustering Model** (Esegui training del **modello di clustering). Collegarne quindi l'output all'input **Untrained model** del modulo **Train Clustering Model**.
...
1. L'algoritmo *K-Means* raggruppa gli elementi nel numero di cluster specificato, ovvero un valore indicato come ***K***. Selezionare il modulo **K-Means Clustering** e nel riquadro a destra impostare il parametro **Number of centroids** su **3**.
    ...
    > 5. Ripetizione dei passaggi c. e d. fino al completamento delle allocazioni del cluster o del numero specificato di iterazioni.
...
1. Selezionare **Configura e invia** ed eseguire la pipeline usando l'esperimento esistente denominato **mslearn-penguin-training** nel cluster di calcolo.

1. Attendere il completamento dell'esecuzione. Il processo può richiedere 5 o più minuti. Controllare lo stato del processo selezionando **Processi** in **Asset**. Selezionare quindi il processo Train Penguin Clustering** più recente**.

1. Al termine dell'esecuzione dell'esperimento, fare clic con il pulsante destro del **mouse sul modulo Assegna dati ai cluster**, selezionare Anteprima dati e quindi selezionare **Set di dati**** Risultati per visualizzare **i risultati.
...
1. Nella pagina Progettazione aprire la bozza della pipeline Train Penguin Clustering.On the **Designer** page, open the **Train Penguin Clustering** pipeline draft.
...
1. Attendere il completamento del processo di esecuzione dell'esperimento. Per verificarne lo stato, passare alla **pagina Processi** e selezionare il processo train Penguin Clustering** più recente**.

1. Fare clic con il pulsante destro del **mouse sul modulo Evaluate Model (Valuta modello** ), selezionare **Preview data (Anteprima dati**), quindi selezionare Evaluation results ( **Risultati** valutazione). Esaminare le metriche in ogni riga:
...
1. Individuare il menu sopra l'area di disegno e selezionare **Crea pipeline** di inferenza. Potrebbe essere necessario espandere lo schermo a schermo intero e fare clic sull'icona **...** nell'angolo in alto a destra dello schermo per trovare **La pipeline** di inferenza nel menu.  
...
1. Nell'elenco **a discesa Crea pipeline** di inferenza selezionare **Pipeline** di inferenza in tempo reale. Dopo alcuni secondi, verrà aperta una nuova pipeline denominata **Train Penguin Clustering-real time inference**.
...
1. Assegnare alla nuova pipeline il nome **Predict Penguin Clusters** e riesaminarla. Le trasformazioni e il modello di clustering nella pipeline di training fanno parte di questa pipeline. Il modello sottoposto a training verrà usato per assegnare un punteggio ai nuovi dati. La pipeline contiene anche un output del servizio Web che restituisce i risultati.
    ...
    - Sostituire il **set di dati penguin-data** con un **componente Enter Data Manually** che non include la **colonna Species** .
    - Rimuovere il **componente Seleziona colonne nel set di dati** , che è ora ridondante.
    - ** ConnessioneInput del servizio** Web e **Immettere manualmente** i componenti (che rappresentano gli input per i dati da raggruppare) al primo **componente Applica trasformazione**.
...
1. La pipeline non include automaticamente un componente **Web Service Input** per i modelli creati dai set di dati personalizzati. Cercare un componente **Web Service Input** nella libreria e posizionarlo nella parte superiore della pipeline.  Connessione l'output dell'oggetto **Componente Input** del servizio Web per l'input *set di dati* (a destra) del primo **componente Applica trasformazione** già presente nell'area di disegno.  
...
1. Connessione l'output dell'oggetto **Immettere manualmente** il modulo Data (Set di dati) (** a destra) del primo **modulo Applica trasformazione**.
...
1. Passare a **Processi** e selezionare il processo Predict Penguin Clusters** più recente **con il nome dell'esperimento **mslearn-penguin-inference**.

1. Al termine della pipeline, fare clic con il pulsante destro del mouse sul **modulo Assegna dati ai cluster**, selezionare **Anteprima dati** e selezionare Set di dati** risultati per visualizzare **le assegnazioni e le metriche previste per i tre pinguini osservazioni nei dati di input.
...
1. Nella pagina Stimare l'inferenza **dei cluster Pinguini selezionare **Distribuisci**** nella barra dei menu in alto.
...
1. Selezionare **Distribuisci un nuovo endpoint** in tempo reale e usare le impostazioni seguenti:
    - **Nome**: predict-penguin-clusters
    - **Descrizione**: pinguini cluster.
...
1. Per visualizzare lo stato della distribuzione, espandere il riquadro sinistro selezionando l'icona del menu in alto a sinistra nella schermata. Visualizzare la pagina **Endpoint** (in **Asset**) e selezionare **predict-penguin-clusters**. Al termine della distribuzione, lo **stato della distribuzione** risulterà **integro**.
