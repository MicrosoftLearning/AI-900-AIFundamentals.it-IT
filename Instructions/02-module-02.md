...
In questo esercizio si userà la funzionalità di Machine Learning automatizzata in Azure Machine Learning per eseguire il training e la valutazione di un modello di Machine Learning. Si distribuirà e testerà quindi il modello sottoposto a training.

Il completamento di questo esercizio richiede circa **30** minuti.
...
> **Suggerimento**: se si ha già un'area di lavoro di Azure Machine Learning, è possibile usarla e passare all'attività successiva.

1. Accedere al [portale di Azure](https://portal.azure.com) usando `https://portal.azure.com` le credenziali Microsoft.

1. Selezionare **+ Crea una risorsa**, cercare *Machine Learning* e creare una nuova **risorsa di Azure Machine Learning** con le impostazioni seguenti:
...
1. In Azure Machine Learning Studio fare clic su **Gestisci funzionalità** di anteprima (icona altoparlante ad alta voce - &#128363;).
...
Machine Learning automatizzato consente di provare più algoritmi e parametri per eseguire il training di più modelli e identificare quello migliore per i dati. In questo esercizio si userà un set di dati di dettagli cronologici sul noleggio di biciclette per eseguire il training di un modello che stima il numero di noleggi di biciclette che devono essere previsti in un determinato giorno, in base alle caratteristiche stagionali e meteorologiche.

> **Citazione**: *i dati usati in questo esercizio derivano da [Capital Bikeshare](https://www.capitalbikeshare.com/system-data) e vengono usati in base al contratto*[ di licenza dati ](https://www.capitalbikeshare.com/data-license-agreement)pubblicato.
...
1. Creare un nuovo processo di Machine Learning automatizzato con le impostazioni seguenti, usando **Avanti** come necessario per eseguire l'avanzamento tramite l'interfaccia utente:
   ...
   **Tipo di attività e dati**:
    ...
    - **Selezionare il set di dati**: creare un nuovo set di dati con le impostazioni seguenti:
        - **Tipo di dati**:
            ...
            - **Descrizione**: Dati storici sul noleggio di biciclette
            - **Tipo**: tabulare
        - **Origine dati**:
        ...
        Selezionare il **set di dati bike-rentals** dopo averlo creato.
    ...
    **** Impostazioni attività:
    ...
    - **Limiti**: *espandere questa sezione*
        ...
        - **Soglia del punteggio** della metrica: 0,85 (*in modo che se un modello ottiene un punteggio di metrica di errore quadratico medio normalizzato pari o inferiore a 0,085, il processo termina.*
        - **Timeout**: 15
        - **Timeout** di iterazione: 5
        - **Abilita terminazione** anticipata: *selezionata*
    - **Convalida e test**:
...
1. Nella scheda **Panoramica** del processo di Machine Learning automatizzato prendere nota del riepilogo del modello migliore.
    ...
    > **Nota** È possibile che venga visualizzato un messaggio con lo stato "Avviso: Il punteggio di uscita specificato dall'utente è stato raggiunto...". Si tratta di un messaggio previsto. Continuare con il passaggio successivo.
...
1. Nella **scheda Modello** per il modello migliore sottoposto a training dal processo di Machine Learning automatizzato selezionare **Distribuisci** e usa l'opzione **Servizio** Web per distribuire il modello con le impostazioni seguenti:
    ...
    - **Descrizione**: Stimare i noleggi di cicli
    - **Tipo di** calcolo: Istanza di Azure Container
    - **Abilita autenticazione**: *selezionata*
...
1. Attendere che lo stato** deploy (Distribuisci) **cambi in *Succeeded (Operazione completata*). L'operazione può richiedere 5-10 minuti.

## Testare il servizio distribuito

A questo punto è possibile testare il servizio distribuito.

1. In studio di Azure Machine Learning, nel menu a sinistra selezionare **Endpoint** e aprire l'endpoint **predict-rentals** in tempo reale.
...
1. Esaminare i risultati del test, che includono un numero stimato di noleggi in base alle funzionalità di input, in modo simile al seguente:
